...
int main(int argc, char* argv[]) {
  ...
  TNamed selection("SelectionSummary",
                   Form("D0 wrong-assignment study: use D0* branches when available, otherwise for each truth D0/D0bar "
                        "choose the final-state Kpi pair minimizing |E(Kpi)-E(D0)|, require |E(Kpi)-E(D0)|<%.3f and "
                        "angle(Kpi,D0)<%.3f, then require GenMatchAngle<%.4f on both legs; compare nominal daughter1->K "
                        "daughter2->pi assignment against swapped daughter2->K daughter1->pi assignment, with kaon-tag "
                        "and optional pion-tag selections, hist range %.3f-%.3f GeV",
