...
int main(int argc, char* argv[]) {
  ...
  TNamed selection("SelectionSummary",
                   Form("Signal-only D0 with correct K/pi assignment from MC: use D0* branches when available, "
                        "otherwise for each truth D0/D0bar choose the final-state Kpi pair minimizing |E(Kpi)-E(D0)|, "
                        "require |E(Kpi)-E(D0)|<%.3f and angle(Kpi,D0)<%.3f, then require GenMatchAngle<%.4f on both "
                        "legs; both RecoGoodTrack==1, both 0.15<=|cos(theta)|<=0.675, opposite charge, daughter1 as "
                        "kaon and daughter2 as pion in mass build, require RecoPIDKaon>=2 on daughter1, and optionally "
                        "RecoPIDPion>=2 on daughter2, hist range %.3f-%.3f GeV",
